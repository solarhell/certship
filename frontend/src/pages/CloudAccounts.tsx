...
import {
  CloudAccountService,
  ListCloudAccountsRequestSchema,
  CreateCloudAccountRequestSchema,
  UpdateCloudAccountRequestSchema,
  DeleteCloudAccountRequestSchema,
  RescanCloudAccountRequestSchema,
  type CloudAccountItem,
} from "@buf/certship_api.bufbuild_es/certship/v1/cloud_account_pb";
...
export default function CloudAccounts() {
  ...
  const rescan = async (id: string, name: string) => {
    ...
    try {
      ...
      const offline = [
        res.missing > 0 ? `${res.missing} 个判定消失` : "",
        res.archived > 0 ? `${res.archived} 个归档` : "",
        res.revived > 0 ? `${res.revived} 个恢复` : "",
      ]
        .filter(Boolean)
        .join("，");
      message.success(
        `${name}:新增 ${res.added} 个域名，更新 ${res.updated} 个，共 ${res.total} 个` +
          (offline ? `；${offline}` : ""),
      );
    } catch (err) {
  ...
  const fetchData = () => {
    setLoading(true);
    client
      .listCloudAccounts(create(ListCloudAccountsRequestSchema, {}))
      .then((res) => setData([...res.accounts]))
      .catch((err) => message.error(`加载云账号失败:${formatError(err)}`))
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editing) {
        await client.updateCloudAccount(
          create(UpdateCloudAccountRequestSchema, { id: editing.id, ...values }),
        );
        ...
        const res = await client.createCloudAccount(
          create(CreateCloudAccountRequestSchema, values),
        );
        ...
        fetchData();
        if (values.enabled) {
          // rescan 内部已处理错误并提示,这里不阻塞创建流程
          void rescan(res.id, values.name);
        }
      }
    } catch {
      /* validation */
      ...
      render: (_, record) => (
        <>
          <Button
            type="link"
            size="small"
            icon={<SyncOutlined spin={rescanningIds.has(record.id)} />}
            disabled={!record.enabled || rescanningIds.has(record.id)}
            onClick={() => void rescan(record.id, record.name)}
          >
            扫描
          </Button>
