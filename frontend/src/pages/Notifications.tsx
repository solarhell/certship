...
export default function Notifications() {
  ...
  const fetchData = () => {
    setLoading(true);
    client
      .listNotificationChannels(create(ListNotificationChannelsRequestSchema, {}))
  ...
  const openModal = (record?: NotificationChannelItem) => {
    ...
    form.setFieldsValue(
      record
        ? { name: record.name, webhookUrl: record.webhookUrl, enabled: record.enabled }
        : { enabled: true },
    );
    setModalOpen(true);
  };
    ...
    try {
      ...
      if (editing) {
        await client.updateNotificationChannel(
          create(UpdateNotificationChannelRequestSchema, { id: editing.id, ...values }),
        );
        message.success("更新成功");
      } else {
        await client.createNotificationChannel(
          create(CreateNotificationChannelRequestSchema, { ...values, type: "feishu" }),
        );
