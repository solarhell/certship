...
import { SyncOutlined, ReloadOutlined, SearchOutlined, StopOutlined } from "@ant-design/icons";
import type { ColumnsType } from "antd/es/table";
import dayjs from "dayjs";
import { createClient } from "@connectrpc/connect";
import { create } from "@bufbuild/protobuf";
import { CertificateService, ListCertificatesRequestSchema, type CertificateItem } from "@buf/certship_api.bufbuild_es/certship/v1/certificate_pb";
import { RenewTaskService, CreateRenewTaskRequestSchema } from "@buf/certship_api.bufbuild_es/certship/v1/renew_task_pb";
...
export default function Certificates() {
  ...
  const handleRenew = async (domains: string[]) => {
    ...
    const results = await Promise.allSettled(
      domains.map((domain) => client.createRenewTask(create(CreateRenewTaskRequestSchema, { domain }))),
    );
    ...
    if (failed === 0) {
      ...
      message.error(`创建失败:${(results[0] as PromiseRejectedResult).reason?.message ?? "未知错误"}`);
    } else {
  ...
  const columns: ColumnsType<CertificateItem> = [
    { title: "域名", dataIndex: "domain", width: 280, render: (v: string, record: CertificateItem) => (
      <Space size={4}>
        <span>{v}</span>
        {record.blockedReason && (
          <Tooltip title={record.blockedReason}>
            <Tag color="volcano" icon={<StopOutlined />} style={{ margin: 0 }}>阻塞</Tag>
          </Tooltip>
        )}
      </Space>
    ) },
