...
import { App, Table, Tag, Card, Select, Space, Tooltip, Button, Modal, Timeline } from "antd";
...
export default function RenewTasks() {
  ...
  const fetchData = useCallback(() => {
    ...
    client
      .listRenewTasks(create(ListRenewTasksRequestSchema, { limit: BigInt(100) }))
      .then((res) => setData([...res.tasks]))
      .catch((err) => message.error(`加载续期任务失败:${formatError(err)}`))
  ...
  const columns: ColumnsType<RenewTaskItem> = [
    { title: "域名", dataIndex: "domain", width: 280, ellipsis: true },
    {
      title: "状态",
      width: 70,
      align: "center",
    ...
    },
    {
      ...
      dataIndex: "trigger",
      render: (t: string) => {
        const m = triggerMap[t as keyof typeof triggerMap];
    ...
    },
    {
      title: "完成时间",
      width: 175,
      render: (_: unknown, r: RenewTaskItem) => {
        ...
        return (
          <>
            {text} <Tag style={{ marginLeft: 4 }}>{dur}</Tag>
          </>
        );
      },
    },
        ...
        {logTask && (
          <Timeline
            ...
            items={logTask.log.map((entry) => ({
              ...
              children: (
                <div>
                  <span style={{ color: "#999", marginRight: 8, fontSize: 12 }}>
