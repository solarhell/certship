...
import { SafetyCertificateOutlined, CheckCircleOutlined, ClockCircleOutlined, ExclamationCircleOutlined } from "@ant-design/icons";
...
export default function Dashboard() {
  ...
  useEffect(() => {
    listCertificates().then((res) => setCerts(res.certificates ?? [])).finally(() => setLoading(false));
  }, []);
  ...
  const columns: ColumnsType<CertificateItem> = [
    ...
    },
    {
      ...
      render: (s: keyof typeof statusMap) => <Tag color={statusMap[s].color}>{statusMap[s].text}</Tag>,
    },
  ];
