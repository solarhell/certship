...
func (d *Daemon) createRenewTasks(ctx context.Context, renewBeforeDays int) {
	...
	for _, dom := range domains {
		...
		// 提前探测 DNS 账号,zone 不可解析就写入 blocked_reason 并跳过,避免反复失败
		if _, _, findErr := alidnsutil.FindAccountForDomain(ctx, d.logger, dom.Domain, allAccounts); findErr != nil {
			_ = dom.Update().SetBlockedReason(findErr.Error()).Exec(ctx)
			d.logger.Info("跳过 DNS 不可解析的域名", zap.String("domain", dom.Domain), zap.Error(findErr))
			continue
		}
		// 探测通过,清掉历史 blocked_reason(如果曾被标记过)
		if dom.BlockedReason != "" {
			_ = dom.Update().SetBlockedReason("").Exec(ctx)
		}
...
// executeTask 执行单个续期任务:申请证书并绑定到域名对应的 OSS/CDN
func (d *Daemon) executeTask(ctx context.Context, task *ent.RenewTask, acmeMgr *acme.Manager) {
	...
	if task.Domain == "" {
		d.failTask(ctx, task, log, tl, fmt.Errorf("任务缺少 domain 字段"))
		return
	}
	...
	dom, err := db.Domain.Query().
		...
		Only(ctx)
	if err != nil {
		d.failTask(ctx, task, log, tl, fmt.Errorf("查询域名 %s 失败: %w", task.Domain, err))
		return
	}
	...
	account, err := db.CloudAccount.Query().
		...
		Only(ctx)
	if err != nil {
		d.failTask(ctx, task, log, tl, fmt.Errorf("找不到云账号 %s: %w", dom.AccountName, err))
		return
	}
	tl.appendf("bucket 所属云账号: %s", account.Name)

	// DNS-01 挑战所用账号可能与 bucket 账号不同,探测 zone 实际所在账号
	allAccounts, err := db.CloudAccount.Query().Where(entcloudaccount.EnabledEQ(true)).All(ctx)
	if err != nil {
		d.failTask(ctx, task, log, tl, fmt.Errorf("查询启用的云账号失败: %w", err))
		return
	}
	dnsAccount, zone, err := alidnsutil.FindAccountForDomain(ctx, d.logger, task.Domain, allAccounts)
	if err != nil {
		d.failTask(ctx, task, log, tl, fmt.Errorf("定位 DNS 账号失败: %w", err))
		return
	}
	if dnsAccount.Name != account.Name {
		tl.appendf("DNS zone %s 在账号 %s 下,使用其 AK 做 DNS-01 挑战", zone, dnsAccount.Name)
	} else {
		tl.appendf("DNS zone %s 就在 bucket 账号下", zone)
	}
	...
	// 申请证书(DNS-01 用 dnsAccount 的 AK)
	tl.appendf("正在申请 Let's Encrypt 证书 (DNS-01)...")
	result, err := acmeMgr.ObtainCert([]string{task.Domain}, dnsAccount.AccessKeyID, dnsAccount.AccessKeySecret, accountKeyPEM, regJSON)
	if err != nil {
	...
	if cdnMgr.IsCDNDomain(account.AccessKeyID, account.AccessKeySecret, dom.Domain) {
		tl.appendf("检测到 %s 为 CDN 域名,部署到 CDN...", task.Domain)
		if err := cdnMgr.DeployCert(account.AccessKeyID, account.AccessKeySecret, dom.Domain, result.CertPEM, result.KeyPEM); err != nil {
			d.failTask(ctx, task, log, tl, fmt.Errorf("部署证书到 CDN %s 失败: %w", task.Domain, err))
			return
		}
		tl.appendf("✓ CDN 证书部署成功: %s", task.Domain)
		// 清理 OSS 侧证书
		ossScanner := oss.NewScanner(d.logger)
		if ossCert := ossScanner.GetDomainCert(ctx, account, dom.Bucket, dom.Region, dom.Domain); ossCert != nil {
			if err := ossBinder.DeleteCert(ctx, domainInfo); err != nil {
				...
				tl.appendf("✓ 已清理 OSS 侧证书: %s", task.Domain)
			}
		}
	} else {
		tl.appendf("检测到 %s 为 OSS 直连域名,绑定到 OSS...", task.Domain)
		if err := ossBinder.BindCert(ctx, domainInfo, result.CertPEM, result.KeyPEM); err != nil {
			d.failTask(ctx, task, log, tl, fmt.Errorf("绑定证书到 OSS %s 失败: %w", task.Domain, err))
			return
		}
		tl.appendf("✓ OSS 证书绑定成功: %s (bucket: %s)", task.Domain, dom.Bucket)
	}
