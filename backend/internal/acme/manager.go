...
// ObtainCert 使用阿里云 DNS-01 挑战申请证书（支持 SAN 多域名）
// accountKeyPEM / registrationJSON 从数据库传入（首次为空），返回的 AccountKeyPEM / RegistrationJSON 应持久化回数据库
func (m *Manager) ObtainCert(domains []string, accessKeyID, accessKeySecret, accountKeyPEM, registrationJSON string) (*CertResult, error) {
	...
	m.logger.Info("开始申请证书", zap.Strings("domains", domains))
	certs, err := client.Certificate.Obtain(certificate.ObtainRequest{
		Domains: domains,
		Bundle:  true,
	})
	if err != nil {
		return nil, fmt.Errorf("obtain cert for %v: %w", domains, err)
	}
	m.logger.Info("证书申请成功", zap.Strings("domains", domains))
...
func (m *Manager) buildUser(accountKeyPEM, registrationJSON string) (*user, error) {
	...
	u := &user{Email: "acme@certship.dev", key: privateKey}
