...
func (s *Server) ListRenewTasks(ctx context.Context, _ *connect.Request[certshipv1.ListRenewTasksRequest]) (*connect.Response[certshipv1.ListRenewTasksResponse], error) {
	tasks, err := database.GetClient().RenewTask.Query().
		Order(renewtask.ByCreatedAt()).
		All(ctx)
	if err != nil {
	...
	return connect.NewResponse(&certshipv1.ListRenewTasksResponse{Tasks: items}), nil
}
