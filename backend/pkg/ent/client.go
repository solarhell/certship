...
type Client struct {
	...
	// RenewTask is the client for interacting with the RenewTask builders.
	RenewTask *RenewTaskClient
...
func (c *Client) init() {
	...
	c.RenewTask = NewRenewTaskClient(c.config)
...
func (c *Client) Tx(ctx context.Context) (*Tx, error) {
	...
	return &Tx{
		...
		RenewTask:           NewRenewTaskClient(cfg),
...
func (c *Client) Mutate(ctx context.Context, m Mutation) (Value, error) {
	...
	case *RenewTaskMutation:
		return c.RenewTask.mutate(ctx, m)
	case *UserMutation:
...
// RenewTaskClient is a client for the RenewTask schema.
type RenewTaskClient struct {
	config
}

// NewRenewTaskClient returns a client for the RenewTask from the given config.
func NewRenewTaskClient(c config) *RenewTaskClient {
	return &RenewTaskClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `renewtask.Hooks(f(g(h())))`.
func (c *RenewTaskClient) Use(hooks ...Hook) {
	c.hooks.RenewTask = append(c.hooks.RenewTask, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `renewtask.Intercept(f(g(h())))`.
func (c *RenewTaskClient) Intercept(interceptors ...Interceptor) {
	c.inters.RenewTask = append(c.inters.RenewTask, interceptors...)
}

// Create returns a builder for creating a RenewTask entity.
func (c *RenewTaskClient) Create() *RenewTaskCreate {
	mutation := newRenewTaskMutation(c.config, OpCreate)
	return &RenewTaskCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of RenewTask entities.
func (c *RenewTaskClient) CreateBulk(builders ...*RenewTaskCreate) *RenewTaskCreateBulk {
	return &RenewTaskCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *RenewTaskClient) MapCreateBulk(slice any, setFunc func(*RenewTaskCreate, int)) *RenewTaskCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &RenewTaskCreateBulk{err: fmt.Errorf("calling to RenewTaskClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*RenewTaskCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for RenewTask.
func (c *RenewTaskClient) Update() *RenewTaskUpdate {
	mutation := newRenewTaskMutation(c.config, OpUpdate)
	return &RenewTaskUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *RenewTaskClient) UpdateOne(_m *RenewTask) *RenewTaskUpdateOne {
	mutation := newRenewTaskMutation(c.config, OpUpdateOne, withRenewTask(_m))
	return &RenewTaskUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *RenewTaskClient) UpdateOneID(id string) *RenewTaskUpdateOne {
	mutation := newRenewTaskMutation(c.config, OpUpdateOne, withRenewTaskID(id))
	return &RenewTaskUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for RenewTask.
func (c *RenewTaskClient) Delete() *RenewTaskDelete {
	mutation := newRenewTaskMutation(c.config, OpDelete)
	return &RenewTaskDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *RenewTaskClient) DeleteOne(_m *RenewTask) *RenewTaskDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *RenewTaskClient) DeleteOneID(id string) *RenewTaskDeleteOne {
	builder := c.Delete().Where(renewtask.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &RenewTaskDeleteOne{builder}
}

// Query returns a query builder for RenewTask.
func (c *RenewTaskClient) Query() *RenewTaskQuery {
	return &RenewTaskQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeRenewTask},
		inters: c.Interceptors(),
	}
}

// Get returns a RenewTask entity by its id.
func (c *RenewTaskClient) Get(ctx context.Context, id string) (*RenewTask, error) {
	return c.Query().Where(renewtask.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *RenewTaskClient) GetX(ctx context.Context, id string) *RenewTask {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// Hooks returns the client hooks.
func (c *RenewTaskClient) Hooks() []Hook {
	return c.hooks.RenewTask
}

// Interceptors returns the client interceptors.
func (c *RenewTaskClient) Interceptors() []Interceptor {
	return c.inters.RenewTask
}

func (c *RenewTaskClient) mutate(ctx context.Context, m *RenewTaskMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&RenewTaskCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&RenewTaskUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&RenewTaskUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&RenewTaskDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown RenewTask mutation op: %q", m.Op())
	}
}
...
type (
	hooks struct {
		AppSettings, AuthToken, CloudAccount, Domain, NotificationChannel, RenewTask,
		User []ent.Hook
	}
	inters struct {
		AppSettings, AuthToken, CloudAccount, Domain, NotificationChannel, RenewTask,
