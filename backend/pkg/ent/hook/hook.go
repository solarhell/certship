...
func (f AppSettingsFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.AppSettingsMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.AppSettingsMutation", m)
}
...
// The RenewTaskFunc type is an adapter to allow the use of ordinary
// function as RenewTask mutator.
type RenewTaskFunc func(context.Context, *ent.RenewTaskMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f RenewTaskFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.RenewTaskMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.RenewTaskMutation", m)
}
