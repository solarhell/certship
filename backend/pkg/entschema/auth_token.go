...
func (AuthToken) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("user_id").Immutable().NotEmpty().Comment("用户 ID"),
		field.String("token").Unique().Immutable().NotEmpty().Comment("Token"),

		field.String("login_user_agent").Immutable().NotEmpty().Comment("登录时的 User-Agent"),
		field.String("login_ip").Immutable().NotEmpty().Comment("登录时的 IP"),
		field.Time("login_time").Immutable().Comment("登录时间"),

		field.String("last_active_user_agent").NotEmpty().Comment("最后活跃的 User-Agent"),
		field.String("last_active_ip").NotEmpty().Comment("最后活跃的 IP"),
